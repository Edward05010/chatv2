...
const Dashboard = () => {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#000', padding: isMobile ? '16px' : '24px', fontFamily: 'sans-serif' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: isMobile ? '20px' : '32px' }}>
        <h1 style={{ fontSize: isMobile ? '24px' : '32px', fontWeight: '700', color: '#fff', margin: 0 }}>Dashboard</h1>
      </div>
          ...
          <div style={styles.graphHeader}>
            ...
            <div style={styles.timeframeButtons}>
              {['day', 'week', 'month'].map(tf => (
                <button key={tf} onClick={() => setTimeFrame(tf)} style={{ ...styles.timeframeBtn, backgroundColor: timeFrame === tf ? '#4CAF50' : 'transparent', color: timeFrame === tf ? '#ffffff' : '#666666', border: timeFrame === tf ? '2px solid #4CAF50' : '2px solid #2a2a2a', fontSize: isMobile ? '12px' : '14px', padding: isMobile ? '6px 12px' : '8px 20px' }}>
          ...
          <div style={styles.graphWrapper}>
            ...
            <div style={styles.graphArea}>
              ...
              <div style={styles.graph}>
                {studyData.map((data, index) => (
                  <div key={index} style={styles.barContainer}>
                    {data.minutes > 0
                      ? <div style={{ ...styles.bar, height: `${(data.minutes / chartMax) * 100}%` }}><div style={styles.barValue}>{data.minutes}</div></div>
              ...
              <div style={styles.xAxisLabels}>
                {studyData.map((data, index) => <div key={index} style={{ ...styles.xLabelWrapper, fontSize: isMobile ? '10px' : '12px' }}>{data.label}</div>)}
              </div>
            </div>
          </div>
        </div>
...
const styles = {
  card: { backgroundColor: '#0a0a0a', border: '1px solid #1a1a1a', borderRadius: '16px', padding: '20px', position: 'relative' },
  ...
  todoInput: { flex: 1, padding: '10px', backgroundColor: '#000', border: '1px solid #1a1a1a', borderRadius: '8px', color: '#fff', fontSize: '14px', outline: 'none' },
  ...
  todoList: { display: 'flex', flexDirection: 'column', gap: '6px', maxHeight: '200px', overflowY: 'auto' },
  ...
  todoLeft: { display: 'flex', alignItems: 'center', gap: '10px' },
  ...
  todoText: { color: '#fff', fontSize: '13px' },
  deleteButton: { background: 'none', border: 'none', color: '#666', cursor: 'pointer', display: 'flex', alignItems: 'center' },
  notificationList: { display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '220px', overflowY: 'auto' },
  ...
  graphWrapper: { display: 'flex', gap: '12px', marginTop: '16px' },
  yAxis: { display: 'flex', flexDirection: 'column', justifyContent: 'space-between', paddingBottom: '55px', minWidth: '32px', paddingTop: '10px' },
  yLabel: { fontSize: '11px', color: '#666', textAlign: 'right' },
  graphArea: { flex: 1, height: '280px', display: 'flex', flexDirection: 'column', position: 'relative' },
  gridLines: { position: 'absolute', top: '10px', width: '100%', height: '220px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', pointerEvents: 'none' },
  gridLine: { width: '100%', height: '1px', backgroundColor: '#1a1a1a' },
  graph: { display: 'flex', alignItems: 'flex-end', justifyContent: 'space-around', height: '220px', borderLeft: '2px solid #2a2a2a', borderBottom: '2px solid #2a2a2a', marginTop: '10px' },
  ...
  barValue: { position: 'absolute', top: '-22px', width: '100%', textAlign: 'center', color: '#fff', fontSize: '11px', fontWeight: 'bold' },
  emptyBar: { height: 0 },
